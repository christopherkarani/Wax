...
enum MCPAgentInstructions {
    static func text(version: String) -> String {
        ...
        1) Call session_open(project?, agent_id?, run_id?, conversation_id?, recall_query?) for a one-shot open. Pass conversation_id = this host chat/session id when the host has one (Grok session UUID) so compaction resumes instead of minting a sibling. The MCP connection remembers the returned session_id; omit it on subsequent calls using that session. Keep the ID for recovery after reconnecting or explicit cross-session calls. Do not invent one. Do not call handoff_latest then session_start as the default open. The same agent_id+run_id resumes the active session. The same conversation_id resumes the unique live match. The same agent_id+resolved project rebinds if exactly one live session; stamp a new run_id; rebound: true. Multiple actives mint a new session — do not guess. After session_open, recall, stats, compact_context, session_resume, and session_close on this MCP connection may omit session_id and still use the live session. If omit-id fails after reconnect, pass the saved UUID or call session_open with the same conversation_id. remember and memory_append inherit session_id unless scope=durable; durable memory types remain durable and receive the session project. An explicit broker-issued session_id overrides the connection default. A later session_open on this connection without a conflicting exact pair resumes the live session. handoff_latest and session_start remain callable for compatibility.
        2) Before answering from memory, call recall. Default scope is project: hard-filters to resolved project (explicit project, session project, or cwd/git root). Empty project lane returns an explicit miss — never auto-widens to global. Pass scope=global only when cross-project retrieval is intentional. For person facts / standing corrections, pass memory_types=["user_preference"] with scope=global so project lessons do not crowd the person lane. Passing session_id merges that session with durable long-term memory under project scope.
        ...
        4) Close when the job ends, not between turns or after compaction. Prefer session_close(content, optional project/pending_tasks/session_id) for atomic handoff+end. Close harvests promotable session facts into durable memory automatically — do not call memory_promote or memory-maintain in the agent loop. Keep content to a concise state summary; put unfinished work only in pending_tasks and never store transcripts. Empty handoff.found is a miss (found=false). Or call handoff then session_end. session_end/session_close `active` is THIS session (false after end). `remaining_active` / `other_sessions_active` / `active_session_count` are other live sessions in the broker, not this one.
