...
enum MCPAgentInstructions {
    ...
    private static func daily(version: String) -> String {
        ...
        Daily tools/list is remember, recall, stats. The server owns sessions and auto-opens one on the first remember or recall. Do not invent a session_id. stats never opens a session.
        1) remember: memory_type selects the horizon. Successful saves return status=ok, committed=true, memory_id, and stored — that is the proof; do not recall again to verify. Default checkout_status is intent for decision and constraint. Pass cwd only — pass cwd once if the error names it.
        ...
        3) Do not close on Stop, idle, or compaction. Durable facts come from explicit remember, not from transcripts.
        """
    }
