...
enum MCPAgentInstructions {
    static func text(version: String) -> String {
        ...
        3) When you learn a fact, call remember with concise content. memory_type selects the horizon: task_state or handoff need top-level session_id (session/working); durable types stay durable even if session_id is present; note is session if session_id is present, else durable. Explicit scope overrides. Never put session_id inside metadata.
