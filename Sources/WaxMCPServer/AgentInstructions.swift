...
enum MCPAgentInstructions {
    static func text(version: String) -> String {
        ...
        1) Call session_open(project?, agent_id?, run_id?, recall_query?) for a one-shot open. Keep the returned session_id. Do not invent one. Do not call handoff_latest then session_start as the default open. The same agent_id+run_id resumes the active session. After session_open, recall and stats on this MCP connection may omit session_id and still use the live session; remember inherits session_id only for task_state, handoff, or scope=session. handoff_latest and session_start remain callable for compatibility.
        ...
        3) When you learn a fact, call remember with concise content. memory_type selects the horizon: task_state or handoff need top-level session_id (session/working); durable types must omit session_id — passing it files them into the session store; note is session if session_id is present, else durable. Explicit scope overrides (scope=durable forbids session_id). Never put session_id inside metadata.
