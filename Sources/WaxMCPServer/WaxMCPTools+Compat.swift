...
// MARK: - Compatibility test path (not production MCP dispatch)
//
// Production MCP calls use the broker-backed `handleCall(params:brokerConfiguration:...)`
// in WaxMCPTools.swift. This file keeps the older direct-MemoryOrchestrator overload and
// helpers so existing unit tests can migrate incrementally to broker-only coverage.
//
// Remaining debt: many tests still exercise this path instead of AgentBrokerService /
// broker-forward MCP. Prefer adding broker-backed tests for new behavior; migrate or
// delete compat coverage when parity is proven.
//
