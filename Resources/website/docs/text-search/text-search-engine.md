...
`FTS5SearchEngine` is a package-only actor that combines SQLite FTS5 full-text search with a complete structured memory system. It is not public API; downstream applications should use the public Wax memory APIs instead. This page documents the implementation surface for Wax contributors.
